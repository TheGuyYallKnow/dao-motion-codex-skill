...
Treat the visible **Target** `TargetRef` input as the canonical single-target connection on flow-driven target consumers. Find Child **Found** carries both success Flow and its found `TargetRef`. When a consumer has exactly one compatible Flow input and one TargetRef input, drop **Found** on **Target**; Motion Lab displays that one wire at **Target**, clears binding warnings, and supplies **In** internally. Dropping it on **In** is accepted as the same gesture, and dropping the already-connected payload again is a no-op. Never add a second visible `target.find-child.outputs.target` edge or a separate `Next` continuation. For every other producer, connect its TargetRef output to **Target** and do not author a hidden `node.target = NodeOutput` descriptor when the live catalog exposes **Target**. Use **Targets** only for a real `TargetSet`, never for Find Child's single result, and use inspector bindings only for fixed Studio instances. Require exactly one of inspector binding, Target, or Targets. Applied runtime metadata may compile the combined Found payload to a NodeOutput descriptor internally.
