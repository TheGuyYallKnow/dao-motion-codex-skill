...
For repeated or changing UI trees, inspect `target.query`, `flow.for-each-target`, `target.find-child`, and `trigger.target-event` before duplicating static motion branches or declaring dynamic targeting unavailable. Use For Each Target to run one Body per discovered target. Use On Target Event only when a genuinely dynamic Query result must own persistent hover, press/release, or focus visual triggers; it never exposes Activated. Bind downstream visual nodes to its Event Target output. Use Button Preset for every known `GuiButton`.
