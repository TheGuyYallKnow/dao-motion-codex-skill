...
1. Within each root or Graph Function scope, group `target.find-child` nodes by parent TargetRef source, activation source, child name, expected class, Yield, and missing-flow contract. Reuse one lookup and fan out its typed target when the complete signature matches.
