...
- Read [reference-cache.md](references/reference-cache.md) before creating, editing, or compacting `target.reference-cache` and `target.reference-linking` nodes.
...
For repeated or changing UI trees, inspect `target.query`, `flow.for-each-target`, `target.find-child`, and the ordinary target triggers before duplicating static motion branches or declaring dynamic targeting unavailable. Prefer the explicit `Query Targets -> For Each Target -> ordinary trigger -> action/call` topology when the user wants a fully editable graph. Treat an unactivated For Each Target as a structural startup root: for every Current Target, execute each passive Body branch once at startup, but use ordinary-trigger Body branches only to install listeners; never fire their downstream actions until the event occurs. Bind For Each Current Target only to each ordinary trigger. Every ordinary target trigger exposes the actual instance that fired as Event Target; bind all downstream actions, conditions, Find Child nodes, and Graph Function TargetRef inputs to that trigger output. For a Graph Function call, connect `trigger.out -> call.in` and `trigger.current` (shown as **Event Target**) `-> call.target` (shown as **Event Target**); ordinary Flow never replaces the separate TargetRef edge, and neither edge may terminate at the other's input. Never bind a downstream node directly to For Each Current Target after an event trigger. `trigger.target-event` / On Each Target Event is a backward-compatible fused shortcut, not the preferred replacement for an explicitly requested For Each plus ordinary-trigger graph. Motion Lab never exposes Activated. Use Button Preset for every known `GuiButton`.

Treat the visible **Target** `TargetRef` input as the canonical single-target connection on flow-driven target consumers. Find Child **Found** exposes one visible gesture backed by separate Flow and hidden found-target outputs. Dropping **Found** on **In** creates only the Flow edge; its found TargetRef is an implicit fallback only while the consumer's compatible **Parent** or **Target** input is unconnected. Dropping **Found** on **Parent** or **Target** creates a separate explicit TargetRef edge from the hidden found-target output and does not create, replace, or satisfy **In** Flow. When the consumer needs both activation and an explicit target, author both edges; the explicit data edge wins over the Flow payload fallback. Never normalize these two gestures into one edge or treat one as the other. Never add a visible `target.find-child.outputs.target` port or a separate `Next` continuation. For every other producer, connect its TargetRef output to **Target** and do not author a hidden `node.target = NodeOutput` descriptor when the live catalog exposes **Target**. Use **Targets** only for a real `TargetSet`, never for Find Child's single result, and use inspector bindings only for fixed Studio instances. Require exactly one target source: inspector binding, explicit Target/Parent data edge, or an eligible In-payload fallback. Applied runtime metadata may compile either found-target route to a NodeOutput descriptor internally.

Use `target.reference-cache` plus `target.reference-linking` for stable graph-scoped reference trees that would otherwise repeat the same named Find Child paths across branches or activations. Inspect both live catalog entries before choosing them. Prefer one Reference Cache with hierarchical entries and one Reference Linking reader per actually used cached entry when several fixed descendants of the graph target or another stable parent are reused, when the same path is resolved more than once, or when a large fixed lookup tree makes the canvas noisy. A cache without incoming Flow initializes at graph start; a connected cache initializes on first execution. Keep `target.find-child` for one-off or branch-specific lookups, and prefer `target.resolve-map` for several required descendants of one activation-local dynamic parent. Never use a graph-scoped cache to smuggle a run-local Event Target across activations or scope boundaries.
...
2. For stable graph-scoped parents, count repeated paths and total fixed descendants before retaining Find Child nodes. When several cached descendants are reused across branches or activations, compare `target.reference-cache` plus `target.reference-linking` and prefer one cache tree over repeated lookups unless the live catalog reports an incompatible scope, class, or missing-reference contract.
3. When one activation needs several required descendants from the same dynamic parent, prefer one `target.resolve-map` over separate `target.find-child` chains. Keep separate lookups when any role is optional or has different missing-flow behavior.
4. Group Motion nodes by scope, activation source, node type, complete normalized parameters, Yield, completion flow, and cancellation ownership. When only explicit bindings differ and the live node cardinality is Many, combine the targets on one node with **Bind Selection** or **Add Selection**.
5. Never reuse a run-local TargetRef across root/Graph Function boundaries or independent activations.
6. Run `graph.validate` and treat `DUPLICATE_TARGET_LOOKUP` and `MERGEABLE_ACTION_TARGETS` warnings as required cleanup. Retain one only when equivalence is impossible, and report the exact semantic difference.
7. Recount nodes and edges after normalization. Do not Stage while an unexplained exact duplicate remains.
...
- Replace a noisy stable graph-scoped Find Child tree with one Reference Cache and only the Reference Linking readers needed by live branches. Preserve each expected class, parent relationship, missing-reference suppression contract, activation scope, and downstream timing.
...
- When Apply rejects a draft before persistence, keep that exact WIP dirty and immediately retryable for the current open editor session, and require the last valid applied runtime to remain unchanged. Treat successful Apply as the durable persistence boundary; never promise that an unapplied WIP survives plugin reload or session end.
